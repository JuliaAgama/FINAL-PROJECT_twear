...
export function CategoryItems(props) {
    ...
    if (isMen) {
        items = CategoriesFilter(categories, 'men').map(item => <Item key={item._id} url={`/categories/men-${item.name}-${item._id}`} title={item.name} mobile={props.mobile} />)
    } else if (isWomen) {
        items = CategoriesFilter(categories, 'women').map(item => <Item key={item._id} url={`/categories/women-${item.name}-${item._id}`} title={item.name} mobile={props.mobile} />)
    }
